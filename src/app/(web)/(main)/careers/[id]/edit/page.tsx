...
export default function EditCareerPage() {
  ...
  const { mutate: updateCareer, isPending } = useUpdateCareer();
  ...
  useEffect(() => {
    if (career && !initialized) {
      setPosition(career.position || "");
      setInstitutionName(career.institutionName || "");
      setProvince(career.province || "");
      if (career.province) {
        setCities(getProvinceCities(career.province));
      }
      setCity(career.city || "");
      ...
      setContactPerson(career.contactPerson || "");
      setContactPhone(career.contactPhone || "");
  ...
  const handleSubmit = () => {
    ...
    updateCareer(
      ...
      {
        onSuccess: () => {
          Notification("success", "Lowongan berhasil diperbarui");
          router.push("/careers");
        },
        onError: (error: any) => {
    ...
    <div className="max-w-full mx-auto">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-slate-500 text-sm">{career?.position}</p>
        </div>
      </div>
